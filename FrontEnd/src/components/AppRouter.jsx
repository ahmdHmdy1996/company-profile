        ...
        try {
          ...
          if (profiles && profiles.length > 0) {
            ...
            if (data.companyName || data.contactInfo) {
              setCompanyData({
                name: Array.isArray(data.companyName)
                  ? data.companyName[0] || ""
                  : data.companyName || "",
                ...
                description: Array.isArray(data.companyDescription)
                  ? data.companyDescription[0] || ""
            ...
            if (data.backgroundImage || data.style?.backgroundColor) {
              setBackgroundSettings({
                ...
                backgroundImage: Array.isArray(data.backgroundImage)
                  ? data.backgroundImage[0] || null
  ...
  const handleUpdateCompanyData = async (changedCompanyData) => {
    try {
      ...
      if (profiles && profiles.length > 0) {
        ...
        console.log(
          "البيانات المرسلة لخدمة الإعدادات:",
          JSON.stringify(companySettingsData, null, 2)
        );
        ...
        const createdProfile = await CompanyProfileService.createProfile(
          newProfile
        );
      ...
      if (profiles && profiles.length > 0) {
        ...
        const backgroundSettingsData = {
          default_background_color:
            changedBackgroundSettings.defaultBackgroundColor || "#ffffff",
          background_image: changedBackgroundSettings.backgroundImage || null,
          background_opacity:
            changedBackgroundSettings.backgroundOpacity || 1.0,
          background_repeat_pattern:
            changedBackgroundSettings.backgroundRepeatPattern || "no-repeat",
        };

        console.log(
          "البيانات المرسلة لخدمة إعدادات الخلفية:",
          JSON.stringify(backgroundSettingsData, null, 2)
        );
        await SettingsService.saveBackgroundSettings(
          lastProfile.id,
    ...
    try {
      ...
      if (profiles && profiles.length > 0) {
        ...
        const themeSettingsData = {
          ...
          background_color:
            changedThemeSettings.colors?.background || "#F8FAFC",
        };

        console.log(
          "البيانات المرسلة لخدمة إعدادات الثيم:",
          JSON.stringify(themeSettingsData, null, 2)
        );
    ...
    try {
      ...
      if (profiles && profiles.length > 0) {
        ...
        const globalSettingsData = {
          ...
          show_page_number:
            changedGlobalSettings.footer?.showPageNumber || true,
          email: changedGlobalSettings.footer?.email || "",
          phone: changedGlobalSettings.footer?.phone || "",
        };

        console.log(
          "البيانات المرسلة لخدمة الإعدادات العامة:",
          JSON.stringify(globalSettingsData, null, 2)
        );
          ...
          element={
            user ? (
              <Routes>
                ...
                <Route path="/pdf-editor/:pdfId" element={<PDFEditor />} />
